...
export async function fetchOpportunitySources(): Promise<OpportunitySource[]> {
  const { data, error } = await supabase
    ...
    .select('id,name,base_url,source_kind,trust_level,status,next_check_at')
    .order('name');
  if (error) throw error;
...
export async function fetchOpportunityIngestionItems(): Promise<OpportunityIngestionItem[]> {
  const { data, error } = await supabase
    ...
    .select('id,source_id,source_url,title,buyer_name,summary,submission_deadline,status,quality_score,quality_issues,duplicate_opportunity_id,opportunity_id,created_at,opportunity_sources(name)')
    .order('created_at', { ascending: false })
    .limit(200);
  if (error) throw error;
...
export async function promoteOpportunityIngestionItem(id: string): Promise<string> {
  const { data, error } = await supabase.rpc('promote_opportunity_ingestion_item', { p_item_id: id });
  if (error) throw error;
...
export async function fetchMyNotifications(options: {
  ...
  return (data ?? []).map((row: any) => ({
    ...
    workspaceTarget: row.workspace_target,
    createdAt: row.created_at,
    readAt: row.read_at,
  }));
}
...
export async function fetchNotificationPreferences(orgId: string): Promise<NotificationPreference[]> {
  ...
  const { data, error } = await supabase
    ...
    .select('category, channel, frequency')
    .eq('user_id', user.id)
    .eq('org_id', orgId);
  if (error) throw error;
...
// ───────────────────── Advert packages and checkout ─────────────────────

export interface AdvertPackage {
  id: string; code: string; name: string; description: string; placement: string;
  prices: Array<{ billingPeriod: 'daily' | 'weekly' | 'monthly'; amount: number; currencyCode: string }>;
}

export interface AdvertOrder {
  id: string; orderNumber: string; invoiceNumber: string; receiptNumber: string | null;
  orgId: string; campaignId: string | null; packageName: string; billingPeriod: string;
  quantity: number; totalAmount: number; currencyCode: string; paymentMethod: string;
  paymentReference: string | null; status: string; createdAt: string;
}
...
export async function fetchAdvertPackages(): Promise<AdvertPackage[]> {
  ...
  if (error) throw error;
  return (data ?? []).map((row: any) => ({
    id: row.id, code: row.code, name: row.name, description: row.description, placement: row.placement,
    prices: (row.advert_package_prices ?? []).map((price: any) => ({
      billingPeriod: price.billing_period, amount: Number(price.amount), currencyCode: price.currency_code,
    })),
  }));
}

export async function fetchAdvertOrders(orgId: string, isPlatformAdmin: boolean): Promise<AdvertOrder[]> {
  let query = supabase.from('advert_orders').select(
...
export async function createAdvertOrder(input: {
  orgId: string; campaignId: string | null; packageId: string; billingPeriod: 'daily' | 'weekly' | 'monthly';
  quantity: number; amount: number; currencyCode: string; paymentMethod: string; paymentReference: string;
}): Promise<void> {
  const { error } = await supabase.rpc('create_advert_order', {
    p_org_id: input.orgId, p_campaign_id: input.campaignId, p_package_id: input.packageId,
    p_billing_period: input.billingPeriod, p_payment_method: input.paymentMethod,
...
export interface AgencyClient {
  id: string; agencyOrgId: string; clientOrgId: string; clientName: string;
  status: string; monthlyBudgetLimit: number | null; permissions: string[];
  reportAccessEnabled: boolean; createdAt: string;
}
export interface AgencyApproval {
  id: string; agencyClientId: string; campaignId: string | null; requestType: string;
  title: string; details: string; status: string; decisionNote: string | null; createdAt: string;
}

export async function fetchAgencyClients(orgId: string): Promise<AgencyClient[]> {
  const { data, error } = await supabase.from('agency_clients')
    ...
    .or(`agency_org_id.eq.${orgId},client_org_id.eq.${orgId}`).order('created_at', { ascending: false });
  if (error) throw error;
  return (data ?? []).map((row: any) => ({
    id: row.id, agencyOrgId: row.agency_org_id, clientOrgId: row.client_org_id,
    clientName: row.organizations?.name ?? 'Client organisation', status: row.status,
    monthlyBudgetLimit: row.monthly_budget_limit == null ? null : Number(row.monthly_budget_limit),
    permissions: row.permissions ?? [], reportAccessEnabled: row.report_access_enabled, createdAt: row.created_at,
  }));
}
...
export async function addAgencyClient(agencyOrgId: string, clientOrgId: string, budget: number | null): Promise<void> {
  ...
  const { error } = await supabase.from('agency_clients').insert({
    agency_org_id: agencyOrgId, client_org_id: clientOrgId, monthly_budget_limit: budget, invited_by: user.id,
  });
  if (error) throw error;
}

export async function updateAgencyClient(id: string, patch: { status?: string; permissions?: string[]; reportAccessEnabled?: boolean; monthlyBudgetLimit?: number | null }): Promise<void> {
...
export async function fetchAgencyApprovals(clientIds: string[]): Promise<AgencyApproval[]> {
  ...
  const { data, error } = await supabase.from('agency_approval_requests')
    .select('id, agency_client_id, campaign_id, request_type, title, details, status, decision_note, created_at')
    .in('agency_client_id', clientIds).order('created_at', { ascending: false });
  if (error) throw error;
  return (data ?? []).map((row: any) => ({
    id: row.id, agencyClientId: row.agency_client_id, campaignId: row.campaign_id ?? null,
    requestType: row.request_type, title: row.title, details: row.details, status: row.status,
...
export async function createAgencyApproval(clientId: string, title: string, details: string): Promise<void> {
  ...
  const { error } = await supabase.from('agency_approval_requests').insert({
    agency_client_id: clientId, request_type: 'campaign_submit', title, details, requested_by: user.id,
  });
  if (error) throw error;
}

export async function decideAgencyApproval(id: string, status: 'approved' | 'changes_requested', note: string): Promise<void> {
  const { data: { user } } = await supabase.auth.getUser();
  const { error } = await supabase.from('agency_approval_requests').update({
    status, decision_note: note || null, decided_by: user?.id ?? null, decided_at: new Date().toISOString(),
  }).eq('id', id);
  if (error) throw error;
}

export async function saveAgencyProfile(orgId: string, input: { tradingName: string; reportBrandName: string; reportPrimaryColor: string; billingEmail: string }): Promise<void> {
  const { error } = await supabase.from('agency_profiles').upsert({
    agency_org_id: orgId, trading_name: input.tradingName, report_brand_name: input.reportBrandName || null,
    report_primary_color: input.reportPrimaryColor, billing_email: input.billingEmail || null, updated_at: new Date().toISOString(),
  });
  if (error) throw error;
}

export async function saveAgencyBulkUpload(clientId: string, fileName: string, rows: Record<string, string>[]): Promise<void> {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error('Please sign in.');
  ...
  const { error } = await supabase.from('agency_bulk_uploads').insert({
    agency_client_id: clientId, file_name: fileName, row_count: rows.length,
    rows, status: 'review', uploaded_by: user.id,
  });
  if (error) throw error;
}
