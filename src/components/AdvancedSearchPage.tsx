    ...
    <main className="min-h-screen bg-[#F8FAFC] text-slate-950">
      <header className="border-b-2 border-slate-950 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4">
          <Link to="/" className="inline-flex items-center gap-2 text-xs font-bold"><ArrowLeft className="h-4 w-4" /> MANOHUB</Link>
      ...
      <section className="border-b-2 border-slate-950 bg-[#0F172A] text-white">
        <div className="mx-auto max-w-7xl px-5 py-10">
          ...
          <h1 className="mt-2 max-w-3xl font-display text-3xl font-extrabold md:text-5xl">Find opportunities, businesses and services across Manohub.</h1>
          <form onSubmit={event => { event.preventDefault(); commitQuery(query); }} className="relative mt-7 max-w-4xl">
            <div className="flex border-2 border-white bg-white text-slate-950">
              ...
              <input aria-label="Search Manohub" value={query} onChange={event => setQuery(event.target.value)} placeholder="Tender, supplier, project, service or influencer" className="min-w-0 flex-1 border-0 px-3 py-4 outline-none" />
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-5 py-7 lg:grid-cols-[280px_1fr]">
        ...
        <section aria-live="polite">
          ...
          {loading && results.length === 0 && <div role="status" className="flex items-center gap-3 border-2 border-slate-950 bg-white p-8 text-sm"><Loader2 className="h-5 w-5 animate-spin" /> Searching trusted Manohub records…</div>}
