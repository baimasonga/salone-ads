...
import { Mail, Lock, User, Building, Landmark, Compass, DollarSign, Globe, Check } from 'lucide-react';
...
export function AuthScreens({ mode, onSwitchMode, onSuccess }: AuthScreensProps) {
  ...
  const handleAuthSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (mode === 'signin') {
      // Direct login success with fallback org
      onSuccess({
        id: 'org-def',
  ...
  const handleOnboardingSubmit = (e: React.FormEvent) => {
    ...
    onSuccess({
      id: `org-${Date.now()}`,
      name: orgName || 'My Salone Enterprise',
      type: orgType,
      country,
        ...
        <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-xl">
          <div className="bg-white py-8 px-6 border-2 border-[#0F172A] rounded-none sm:px-10">
            <form onSubmit={handleOnboardingSubmit} className="space-y-6 text-left">
              ...
              <button
                type="submit"
                className="btn-geometric w-full flex justify-center cursor-pointer mt-4"
              >
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-6 border-2 border-[#0F172A] rounded-none sm:px-10">
          <form className="space-y-6 text-left" onSubmit={handleAuthSubmit}>
            ...
            <button
              type="submit"
              className="btn-geometric w-full flex justify-center cursor-pointer"
            >
              {mode === 'signin' ? 'Sign In' : 'Continue to Onboarding'}
            </button>
          </form>

          {/* Social provider placeholder for Google Auth requirement */}
          <div className="mt-6 border-t border-[#0F172A] pt-6">
            <button
              onClick={() => {
                onSuccess({
                  id: 'org-def',
                  name: 'Freetown Agrotech',
                  type: 'Small Business',
                  country: 'Sierra Leone',
                  district: 'Western Area Urban',
                  primaryObjective: 'WhatsApp enquiries',
                  monthlyBudget: 'Le 20,000,000'
                });
              }}
            ...
            >
              ...
              <span>Google Sign-In Callback</span>
            </button>
          </div>
        </div>
      </div>
