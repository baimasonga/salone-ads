...
import { unsubscribePublicAudience } from '../lib/procurementApi';
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#172554] px-6 py-12 text-[#0F172A]">
      <section className="w-full max-w-lg border-2 border-[#0F172A] bg-white p-8 text-center shadow-[10px_10px_0_0_#F4D35E]">
        ...
        {state === 'done' && <><CheckCircle className="mx-auto h-12 w-12 text-emerald-600" /><h1 className="mt-5 font-display text-2xl font-extrabold">You have been unsubscribed.</h1><p className="mt-3 text-sm text-slate-600">Manohub will no longer send audience updates to this subscription.</p></>}
        ...
        {state === 'error' && <><ShieldCheck className="mx-auto h-12 w-12 text-red-600" /><h1 className="mt-5 font-display text-2xl font-extrabold">We could not process the request.</h1><p className="mt-3 text-sm text-slate-600">Please try again later or contact Manohub support.</p></>}
        <Link to="/" className="mt-7 inline-flex border border-[#0F172A] bg-[#0F172A] px-5 py-3 font-mono text-xs font-bold uppercase tracking-widest text-white">Return to Manohub</Link>
      </section>
    </main>
  );
}
