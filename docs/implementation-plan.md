# Hyderra Implementation Plan

> **⚠️ Superseded by a product pivot — read this first.** This document describes the *original*
> ad-platform-only vision. Hyderra has since pivoted: **tenders/procurement are now the subscriber-
> facing product**, and everything below (Campaigns, Content Studio, Media Library, Calendar, Tracking,
> Directory, Influencers, Events, Tourism) is now **admin-only internal tooling** used by the Hyderra
...
| **Phase 6** | **Tracking & CRM Leads** | Tracking short-links, WhatsApp clicks, QR codes, and lightweight lead pipelines | **Done** — real short links (`tracking_links`/`tracking_link_clicks` + a genuine `/r/:code` server redirect that logs clicks), high-resolution downloadable QR codes, and real CRM lead creation. |
